...
fn main() {
    ...
    let mut noise_writer = hound::WavWriter::create(out_path, orig_spec).expect("Çıktı dosyası oluşturulamadı");
    ...
    println!("├─ Ortalama Gürültü Sapması: {:.2} (Maks 32767 üzerinden)", avg_noise);
    println!("├─ Maksimum Gürültü Zirvesi: {}", max_noise);
    println!("└─ Sadece gürültüyü içeren dosya '{}' olarak kaydedildi.", out_path);
